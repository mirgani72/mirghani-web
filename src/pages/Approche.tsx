    ...
    <main>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-14 pb-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
        {STEPS.map(({ Icon, n, key }) => (
          <div key={n}>
            <Icon className="w-12 h-12 text-navy mb-4" strokeWidth={1.5} />
            <p className="font-display text-sm text-gold mb-2">{n}</p>
      ...
      <section className="bg-white">
        <div className="max-w-6xl mx-auto px-6 pt-12 pb-16 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <img src={PHOTOS.street} alt="Rue résidentielle avec verdure" className="w-full aspect-[4/3] object-cover rounded-sm" />
          <div>
